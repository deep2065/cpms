...
converttopdf(){
  ...
  this.vendorcost.forEach(ve=>{
    paragraph+=itemno +". "+ve.item+" \n ";
    itemtotal+= parseFloat(ve.vcost);
    itemno++; 
  });
 ...
 /* var doc = new jsPDF('p','pt','a4');
  ...
  doc.text(40,50,data.firstparagraph);
  doc.text(50,80,data.p1);
  doc.text(50,95,data.p2);
  doc.text(50,110,data.p3);
  doc.text(50,125,data.p4);
  doc.text(40,140,data.sp);
  doc.text(40,155,data.sp1);
  doc.text(40,175,data.sp2);
  doc.text(40,190,data.sp3);
  doc.output("dataurlnewwindow");

  /*const elementToPrint = document.getElementById('htmltopdf'); //The html element to become a pdf
  let pdf = new jsPDF('p', 'pt', 'a4');
  let options = {
     pagesplit: true
  };
  pdf.addHTML(elementToPrint, 0.5, 0.5, options, () => {
     pdf.save("test.pdf");
  });*/
}

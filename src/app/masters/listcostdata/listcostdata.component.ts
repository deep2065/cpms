...
@Component({
  selector: '[listcostdata]',
  templateUrl: './listcostdata.template.html',
  styleUrls: [ './listcostdata.style.scss' ],
  encapsulation: ViewEncapsulation.None,
})
export class Listcostdata {
...
console.log(data);
    ...
    onDeleteConfirm(event,data:any) {
      if (window.confirm('Are you sure you want to delete?')) {
        ...
        this.db.list('/costdatas/'+data+'/material').$ref.ref.child(key).remove();
    ...
    onSaveConfirm(event,data:any) {
      if (window.confirm('Are you sure you want to update?')) {
        var key:any = event.data.key-1;
       this.db.list('/costdatas/'+data+'/material').$ref.ref.child(key).set(event.newData);
